...
ISR(ADC_vect) {
	if (adc_full == false) {
		if (voltage_counter < SAMPLING_SIZE) {
			adc_voltage[voltage_counter] = adc_convert_mV(adc_read(ADC_V));
			...
			adc_current[current_counter] = adc_convert_mV(adc_read(ADC_C)); //conversion for mA, needs adjustment
			...
			adc_temp1[temp1_counter] = adc_convert_mV(adc_read(ADC_T1)); //conversion for temp different, needs adjustment
			//check for rated value and cut
			...
			adc_temp2[temp2_counter] = adc_convert_mV(adc_read(ADC_T2)); //conversion for temp different, needs adjustment
			//check for rated value and cut
			...
			adc_temp3[temp3_counter] = adc_convert_mV(adc_read(ADC_T3)); //conversion for temp different, needs adjustment
			//check for rated value and cut
